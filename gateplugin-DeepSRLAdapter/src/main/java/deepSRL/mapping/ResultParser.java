...
public class ResultParser {
	...
	private static void parseSRL (Sentence sentence, String line) {
		...
		int verbcount = 0;
//		line.replaceAll(SENTENCE_START_REGEX, "");
//		line.replaceAll(SENTENCE_END_REGEX, "");
		...
		while(predicateIterator.hasNext()) {
			...
			while (tokenIterator.hasNext()) {
				...
				for(int tokennumber = Integer.valueOf(tokenLabels.get(SRL_ARG_START)); tokennumber<=Integer.valueOf(tokenLabels.get(SRL_ARG_END)); tokennumber++) {
					sentence.getTokens().get(tokennumber).srlValues.add(tokenLabels.get(SRL_ARGUMENT_TYPE).replaceAll(CHAR_MARKS, ""));
				}
				tokenLabels.clear();
			}
